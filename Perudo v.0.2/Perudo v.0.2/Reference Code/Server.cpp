...
bool Server::handleClientMessage(sf::TcpSocket* client) {
    ...
    // Every packet is a single command string (e.g., "HELLO alice", "ROLL", "BET 3 4", "DOUBT", "NEXT")
    std::string line;
    ...
    if (line == "ROLL") {
        // First person to roll becomes default firstRoundStarter (only if empty)
        if (turnOrder.empty()) firstRoundStarter = nameOf(client);

        std::cout << "Server: ROLL from " << nameOf(client) << "\n";

        setupTurnOrderIfNeeded();

        // Deal new dice to everyone
        rollAllDice();

        // Announce phase BEFORE sending MYDICE so clients accept dice in BETTING
        ...
        // Now privately send each player's dice
        sendPrivateDiceToOwners();

        // Kick off betting turn
    ...
    // Unknown (keep logging small to avoid spam)
    std::cout << "Server: unknown line: " << line << "\n";
    return true;
}
...
void Server::startBettingIfPossible() {
    ...
    while (!turnOrder.empty() &&
        playersBySock[turnOrder[turnIndex]].diceCount <= 0 &&
        attempts < (int)turnOrder.size()) {
...
void Server::beginNextRound() {
    ...
    // announce phase BEFORE sending dice
    broadcast("PHASE BETTING");
    phase = Phase::Betting;
    ...
    // choose opener = last round loser if available, otherwise the original firstRoundStarter
    if (!turnOrder.empty()) {
